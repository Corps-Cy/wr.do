...
-- 添加索引
CREATE INDEX idx_domains_dns_provider ON domains(dns_provider);
CREATE INDEX idx_platform_configs_domain_provider ON platform_configs(domain_id, provider);

-- 更新现有域名的 DNS 提供商
UPDATE domains SET dns_provider = 'cloudflare' WHERE enable_dns = true AND dns_provider IS NULL;
